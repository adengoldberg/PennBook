 ...
 */
 ...
 //Extension of the given KVS given to store post data 
  ...
  keyvaluestore.prototype.get = function(search, callback) {
    ...
    if (self.cache.get(search))
      ...
      var params = {
          ...
          TableName: self.tableName,
          AttributesToGet: [ 'inx', 'message', 'poster', 'time' ]
      };
      ...
      db.query(params, function(err, data) {
        if (err || data.Items.length == 0) {
          ...
          for (var i = 0; i < data.Items.length; i++) {
            items.push({"inx": data.Items[i].inx.N, "message": data.Items[i].message.S,  "poster": data.Items[i].poster.S, "time": data.Items[i].time.S});
          }
